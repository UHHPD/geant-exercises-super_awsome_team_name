...
public:
  ...
  double pt() const { 
    return 0.3 * B() * r()/ 100.0;
}//needs changes

  double rErr() const { return sqrt(fCov(0,0));}
  double ptErr() const { 
    
    return 0.3 * B() * rErr()/100.0;
}//needs changes
  ...
  double x(double lambda) const { 
    double ret = x0() + r() * charge() * sin(charge() * lambda + phi0());
    return ret;
}//needs changes
  double z(double lambda) const { 
    double ret = z0() - r() * charge() *cos(charge()* lambda + phi0());
    return ret;
}//needs changes
...
void tracking5()
{
  res->SetTitle("distance vs Resolution");
  ...
  TutorialApplication* app = (TutorialApplication*)TutorialApplication::Instance();
  bool doFit = true;
  double distance_values[] = {30, 20, 10, 0, -10, -20, -30};
  for(unsigned int ii =0 ;ii<5; ii++){
  // position of silicon layers in x   
  double pos1 = -45.0;
  double pos2 = distance_values[ii];
  double pos3 = pos2 + 15.0; 
  double pitch = 0.0150;
  ...
  res->SetPoint(ii,pos2, hpt->GetRMS());
  }
